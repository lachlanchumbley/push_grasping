...
def main():
    ...
    for i, bag_dir in enumerate(bag_list):
        ...
        force = [msg for _, msg, _ in bag.read_messages(topics=['force'])]
        xpos = [msg for _, msg, _ in bag.read_messages(topics=['x_pos'])]

        print(len(time_data),len(rgb_image),len(depth_image),len(action), len(force), len(xpos))
        
        if True: #len(time_data) == len(rgb_image) == len(depth_image) == len(action):
            ...
            for i, (time, rgb, depth, a, r, f, x) in enumerate(itertools.izip(time_data, rgb_image, depth_image, action, reward, force, xpos)):
            # num_data_points += 1
                ...
                print("action", a, "reward", r, "force", f, "xpos", x)
                cv2.waitKey(10)
        else:
